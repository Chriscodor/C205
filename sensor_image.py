...
def image(image):
   ...
   matrix_data = np.arry(image.raw_data)
   reshape_of_image = matrix_data.reshape((IM_HEIGHT, IM_WIDTH, 4))
   lffc = reshape_of_image[:,:,:3]
   cv2.imshow("", lffc)
   cv2.waitKey(1)
   return
...
try:
    ...
    car_model = getblueprint_of_world.filter('mode13')[0]
    ...
    dropped_vehicle.apply_control(clara.VehicleControl(throttle=1.0, steer=1.0))
